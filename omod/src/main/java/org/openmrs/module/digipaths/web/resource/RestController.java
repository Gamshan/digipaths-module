...
public class RestController extends MainResourceController {
	...
	public void getHemoglobinAction(DepartmentService departmentService, String patientUuid, List<Map<String,Object>> list) {
		...
		if(!(existDiabetes(departmentService, patientUuid) || existType2Diabetes(departmentService,patientUuid)))
			return;
	...
	public void getAlbuminRatioAction(DepartmentService departmentService, String patientUuid, List<Map<String, Object>> list) {
		...
		Context.getOrderService().getAllOrdersByPatient(new Patient(123)).stream().anyMatch(e-> {

			return e.getConcept() != null && e.getConcept().equals(aceInhibitorConcept);
		});


		
		boolean existTBD2 = existType2Diabetes(departmentService, patientUuid);
		boolean existAceInhibitor = aceInhibitorConcept != null
		        && departmentService.existOrderByPatientUuidAndConceptId(patientUuid, aceInhibitorConcept.getId());
		...
		for (Obs o1 : obsList) {
			for (Obs o2 : obsList) {
				...
				if (diffDays < 90 && patient != null && patient.getGender() != null && patient.getAge() != null) {
					...
					String recommendationLessThan45 = "<p>If the eGFR is below 45 mL/min/1.73m², the severity warrants timely referral to a nephrologist for specialized evaluation and management to slow disease progression and address complications.</p>";
					String recommendationLessThan60 = "<p>The patient has had two separate serum creatinine tests, spaced at least three months apart, both indicating estimated eGFR values below 60 mL/min/1.73m².</br> This persistent reduction in kidney function suggests the presence of Chronic Kidney Disease (CKD), as per clinical guidelines.</p>";

					if(o1GFR < 45) {
						list.add(getDigipathsRow(o1.getObsDatetime(), messageLessThan45, "<h6> eGFR: " + String.format("%.1f", o1GFR)  + " mL/min/1.73m² </h6>" + recommendationLessThan45));
						 messageAdded = true;
					} else if(o2GFR < 45) {
						list.add(getDigipathsRow(o2.getObsDatetime(), messageLessThan45, "<h6> eGFR: " + String.format("%.1f", o2GFR) + " mL/min/1.73m² </h6>" + recommendationLessThan45));
						messageAdded = true;
					}

					if(!existCKD && o1GFR < 60){
						list.add(getDigipathsRow(o1.getObsDatetime(), messageLessThan60, "<h6> eGFR: " + String.format("%.1f",  o1GFR) + " mL/min/1.73m² </h6>" + recommendationLessThan60));
						messageAdded = true;
					} else if(!existCKD && o2GFR < 60) {
						list.add(getDigipathsRow(o2.getObsDatetime(), messageLessThan60,"<h6> eGFR: " + String.format("%.1f", o2GFR) + " mL/min/1.73m² </h6>" +  recommendationLessThan60));
						messageAdded = true;
					}
				}
				if(messageAdded) {
